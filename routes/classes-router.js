...
// Remove an empty punch card
router.delete('/delete_pass/:class_id', restricted, (req, res) => {
	const class_id = req.params.class_id;
	const { user_id } = req.decodedJwt;
	...
	return db('punch_cards')
		...
		.then(deleted_count => {
			if (!deleted_count) {
				res.status(403).json({ message: "punch card isn't empty!" });
			} else {
				res.status(201).json({ message: 'punch card cleared from database' });
			}
		})
		.catch(err => {
			return status(500).json(err);
